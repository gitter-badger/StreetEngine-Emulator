...
namespace StreetEngine.Engine.Network
{
    ...
    public class mmoServer
    {
        ...
        public static String ip = EngineConfig.IniConfig.IniValues.h_world;
        public static Int32 port = EngineConfig.IniConfig.IniValues.p_world;
        ...
        public static void ServerOnAcceptSocketEvent(SilverSocket socket)
        {
            try
            {
                mmoClient session = new mmoClient(socket);
            }            
            catch (Exception ex) { error(ex.ToString()); }
        }
        ...
        public static void ServerOnListeningEvent()
        {
            try
            {
                evnt("mmo_server@" + ip + ":" + " server started");
            }
            catch (Exception ex) { error(ex.ToString()); }
        }
    }
